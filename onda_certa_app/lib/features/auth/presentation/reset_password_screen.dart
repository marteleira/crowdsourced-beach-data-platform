...
class _ResetPasswordScreenState extends ConsumerState<ResetPasswordScreen> {
  ...
  // 0 = code, 1 = new password, 2 = success
  int _step = 0;
  String _code = '';
  ...
  Future<void> _resend() async {
    ...
    final errMsg = l10n.codeResendError;
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(detail is String ? detail : errMsg)),
        );
      }
    } catch (_) {
  ...
  void _advanceToPassword() {
    setState(() { _code = _codeKey.currentState!.code; _step = 1; _errorMessage = null; });
  }

  Future<void> _submit() async {
    final l10n = context.l10n;
    final errMsg = l10n.resetPasswordError;
    final connMsg = l10n.connectionError;
    if (!(_formKey.currentState?.validate() ?? false)) return;
    setState(() { _loading = true; _errorMessage = null; });
    try {
      await ref.read(authRepositoryProvider).resetPassword(widget.email, _code, _passwordCtrl.text);
      if (mounted) setState(() => _step = 2);
    } on DioException catch (e) {
      if (e.type == DioExceptionType.cancel) return;
      final detail = e.response?.data?['detail'];
      final msg = detail is String ? detail : errMsg;
      if (detail is String && (detail.contains('Código') || detail.contains('expirado'))) {
        _codeKey.currentState?.clear();
        setState(() { _code = ''; _codeComplete = false; _step = 0; _errorMessage = msg; });
      } else {
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      onPopInvokedWithResult: (didPop, _) {
        if (!didPop) setState(() { _step = 0; _errorMessage = null; });
      },
      child: Scaffold(
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: const Icon(Icons.mark_email_read_outlined, size: 36, color: AppColors.tealDark),
          ),
          const SizedBox(height: AppSpacing.xxl),
          Text(l10n.resetCodeTitle,
            style: Theme.of(context).textTheme.headlineMedium?.copyWith(color: AppColors.primary, fontWeight: FontWeight.w700)),
          const SizedBox(height: AppSpacing.sm),
          Text(l10n.resetCodeBody(widget.email),
          ...
          Align(
            ...
            child: TextButton(
              ...
              child: _resending
                  ...
                  : Text(
                      _cooldown > 0 ? l10n.codeResendShortCooldown(_cooldown) : l10n.codeResend,
                      style: TextStyle(
                        fontSize: 13,
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: Text(l10n.resetCodeContinue,
    ...
    return SingleChildScrollView(
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            const SizedBox(height: AppSpacing.lg),
            Text(l10n.resetNewPasswordTitle,
              style: Theme.of(context).textTheme.headlineMedium?.copyWith(color: AppColors.primary, fontWeight: FontWeight.w700)),
            const SizedBox(height: AppSpacing.sm),
            Text(l10n.resetNewPasswordBody,
              style: const TextStyle(fontSize: 15, color: AppColors.textSecondary, height: 1.6)),
            const SizedBox(height: AppSpacing.xxxl),
            ...
            TextFormField(
              ...
              obscureText: true,
              validator: (v) {
                if (v == null || v.isEmpty) return l10n.codeConfirmEmpty;
                if (v != _passwordCtrl.text) return l10n.accountPasswordMismatch;
                return null;
              },
              style: const TextStyle(color: AppColors.primary),
              decoration: authInputDecoration(
                label: l10n.resetNewPasswordConfirmLabel,
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: _loading
                    ...
                    : Text(l10n.resetNewPasswordSubmit,
                        style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
              ),
            ),
            const SizedBox(height: AppSpacing.xxxl),
      ...
      child: Column(
        ...
        children: [
          Align(
            child: Container(
              width: 80, height: 80,
              decoration: BoxDecoration(color: AppColors.flagGreen.withValues(alpha: 0.12), shape: BoxShape.circle),
              child: const Icon(Icons.check_circle_outline, size: 44, color: AppColors.flagGreen),
            ),
          ),
          const SizedBox(height: AppSpacing.xxl),
